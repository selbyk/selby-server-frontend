...
export default Ember.Component.extend({
  ...
  getPath: function() {
    ...
    if (!this.get('path')) {
      ...
      Ember.$.getScript('https://cdnjs.cloudflare.com/ajax/libs/d3/3.5.5/d3.min.js', function(data, textStatus) {
        _this.set('path', d3.select('#arc-' + _this.get('elementId')).data(_this.get('value')));
      });
  ...
  update: function() {
    if(this.get('path')){
      ...
      this.get('path').datum(endAngle);
      this.get('path').transition().duration(_duration)
        .attrTween("d", arcTween);

      if (ratio > 1) {
        this.get('path2').datum(Math.min(360 * (ratio - 1), 360) * Math.PI / 180);
        this.get('path2').transition().delay(_duration).duration(_duration)
  ...
  setupSvg: function() {
    ...
    Ember.$.getScript('https://cdnjs.cloudflare.com/ajax/libs/d3/3.5.5/d3.min.js', function(data, textStatus) {
      _this.$('svg path.radial-arc').attr("id", 'arc-' + _this.get('elementId'));
      _this.$('svg path.radial-arc2').attr("id", 'arc2-' + _this.get('elementId'));
      _arc = d3.svg.arc()
      ...
      _this.get('path').enter().append("path")
        .attr("class", "radial-arc")
        .attr("transform", "translate(" + _width / 2 + "," + _width / 2 + ")")
        .attr("d", _arc);

      //Another path in case we exceed 100%
      _this.set('path2', svg.select('#arc2-' + _this.get('elementId')).selectAll(".radial-arc2").data(_this.get('value')));
      _this.get('path2').enter().append("path")
